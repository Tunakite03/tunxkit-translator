...
export class AssemblyAIClient {
  ...
  connect(config: AssemblyAIConfig): void {
    ...
    if (!config.apiKey) {
      this._setStatus("error");
      this.onError?.(
        "AssemblyAI API key is required. Please add it in Settings.",
      );
      return;
    }
  ...
  private async _doConnect(config: AssemblyAIConfig): Promise<void> {
    ...
    try {
      tempToken = await invoke<string>("get_assemblyai_token", {
      ...
      if (errMsg.includes("Invalid AssemblyAI API key")) {
        this.onError?.(
          "Invalid AssemblyAI API key. Please check your key in Settings.",
        );
      } else {
    ...
    const speechModel =
      config.speechModel || "universal-streaming-multilingual";
    ...
    // Enable language detection for the multilingual models when no specific lang
    if (
    ...
    newWs.onclose = (event: CloseEvent) => {
      console.log(
        "[AssemblyAI] WebSocket CLOSED, code:",
        event.code,
        "reason:",
        event.reason,
      );
      ...
      if (event.code === 1000) {
        ...
        this._setStatus("error");
        this.onError?.(
          "Free tier session limit reached. Upgrade your AssemblyAI plan.",
        );
      } else {
  ...
  sendAudio(pcmData: ArrayBuffer): void {
    if (this.ws?.readyState === WebSocket.OPEN) {
      ...
      if (this._audioSendCount <= 5 || this._audioSendCount % 50 === 0) {
        console.log(
          `[AssemblyAI] sendAudio #${this._audioSendCount}, bytes:`,
          pcmData.byteLength,
        );
      }
  ...
  private _startUtteranceDebounce(): void {
    if (this._utteranceDebounceTimer)
