...
export interface AppSettings {
  ...
  /** Context window size for LLM translation (number of recent sentences to include) */
...
class SettingsManager {
  ...
  async load(): Promise<AppSettings> {
    try {
      ...
      if (!this.settings.tts_enabled && this.settings.tts_auto_read) {
        this.settings.tts_auto_read = false;
      }
    } catch (err) {
