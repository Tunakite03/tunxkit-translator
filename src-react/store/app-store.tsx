...
function resolveActiveSpeechToTextModel(
    ...
    case "assemblyai":
      return (
        ...
        getDefaultSpeechToTextModel(
          "assemblyai",
          settings.source_language,
          getFallbackSttModels("assemblyai"),
        )
      );
    default:
      return settings.local_asr_model || "whisper";
  }
}
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [provisionalSpeaker, setProvisionalSpeaker] = useState<string | null>(
    null,
  );
  ...
  const getFormattedContent = useCallback(
    (metadata: Record<string, string> = {}) => {
      ...
      if (metadata.sourceLang)
        lines.push(`source_language: ${metadata.sourceLang}`);
      if (metadata.targetLang)
        lines.push(`target_language: ${metadata.targetLang}`);
      if (metadata.duration)
        lines.push(`recording_duration: ${metadata.duration}`);
      if (metadata.audioSource)
        lines.push(`audio_source: ${metadata.audioSource}`);
  ...
  const configureTTS = useCallback(
    (tts: typeof elevenLabsTTS | typeof edgeTTSRust, s: AppSettings) => {
      const provider = s.tts_provider || "edge";
  ...
  const toggleTTS = useCallback(() => {
    ...
    setTtsEnabled((prev) => {
      ...
      if (next) {
        ...
        showToast(
          `TTS narration ON (${label[provider] || provider})`,
          "success",
        );
      } else {
        tts.disconnect();
  ...
  const saveTranscript = useCallback(async () => {
    ...
    const content = getFormattedContent({
      model:
        ...
        translationModeRef.current === "local"
          ? getSpeechToTextModelLabel(
              translationModeRef.current as SpeechToTextProvider,
              resolveActiveSpeechToTextModel(
                s,
                translationModeRef.current as SpeechToTextProvider,
              ),
            )
  ...
  const saveTranscriptAs = useCallback(async () => {
    ...
    try {
      const savedPath = await invoke<string>("save_transcript_as", {
        path: filePath,
        content,
      });
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      if (mode === "assemblyai" && !s.assemblyai_api_key) {
        showToast(
          "AssemblyAI API key is required. Add it in Settings.",
          "error",
        );
        setView("settings");
        return;
      ...
      if ((s.translation_engine || "mymemory") === "llm" && !s.llm_api_key) {
        showToast(
          "LLM API key is required for AI translation. Add it in Settings.",
          "error",
        );
        setView("settings");
        return;
  ...
  const startDeepgramMode = useCallback(
    async (s: AppSettings) => {
      ...
      deepgramClient.connect({
        ...
        model: resolveActiveSpeechToTextModel(s, "deepgram"),
  ...
  const startAssemblyAIMode = useCallback(
    async (s: AppSettings) => {
      ...
      assemblyAIClient.connect({
        ...
        speechModel: resolveActiveSpeechToTextModel(s, "assemblyai"),
        sourceLanguage: s.source_language,
        targetLanguage: s.target_language,
        translationEngine: s.translation_engine || "mymemory",
        customContext: s.custom_context ?? undefined,
    ...
    async (s: AppSettings) => {
      ...
      try {
        ...
        await invoke("start_local_pipeline", {
          asrModel: resolveActiveSpeechToTextModel(s, "local"),
          sourceLang: s.source_language || "auto",
          targetLang: s.target_language || "vi",
      ...
      try {
        ...
        audioChannel.onmessage = async (pcmData: unknown) => {
          try {
            await invoke("send_audio_to_pipeline", {
              data: Array.from(new Uint8Array(pcmData as ArrayBuffer)),
            });
          } catch {}
        };
        await invoke("start_capture", {
  ...
  const handleLocalPipelineResult = useCallback(
    (data: LocalPipelineData) => {
      switch (data.type) {
        ...
        case "status":
          setStatusText(
            data.message?.replace(/^\[pipeline\]\s*/, "") || "Loading...",
          );
          break;
        case "done":
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      if (mounted) {
        setSettings(s);
        setCurrentSource(
          s.audio_source === "both" ? "system" : s.audio_source || "system",
        );
      ...
      try {
        ...
        if (mounted)
          setIsAppleSilicon(info.os === "macos" && info.arch === "aarch64");
      } catch {
      ...
      assemblyAIClient.onProvisional = (
        text: string,
        speaker: string | null,
      ) => {
        if (mounted) {
  ...
  const handleSaveSettings = useCallback(
    async (newSettings: Partial<AppSettings>) => {
      try {
        ...
        setCurrentSource(
          merged.audio_source === "both"
            ? "system"
            : merged.audio_source || "system",
        );
        const provider = merged.tts_provider || "edge";
        const needsKey =
          provider === "elevenlabs" && !merged.elevenlabs_api_key;
