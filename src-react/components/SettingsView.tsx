      ...
      setForm((prev) => {
         ...
         if (key === 'llm_api_key' && typeof value === 'string') {
            ...
            const isDefaultOrEmpty =
               ...
               prev.llm_base_url === 'https://generativelanguage.googleapis.com/v1beta/openai';
            if (isDefaultOrEmpty) {
               if (value.startsWith('gsk_')) {
                  next.llm_base_url = 'https://api.groq.com/openai/v1';
               } else if (value.startsWith('AIza')) {
                  next.llm_base_url = 'https://generativelanguage.googleapis.com/v1beta/openai';
   ...
   const handleSave = async () => {
      ...
      const customContext =
         form.context_domain || terms.length > 0
            ? { domain: form.context_domain || null, translation_terms: terms.filter((t) => t.source && t.target) }
            : null;
