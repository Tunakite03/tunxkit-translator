...
export default function TranslationTab({ form, update, terms, setTerms, sourceLanguages, targetLanguages }: Props) {
      ...
      <div className='space-y-5'>
         ...
         <SettingsSection>
            ...
            >
               ...
               <SelectContent>
                  ...
                  <SelectItem value='local'>🖥️ Local MLX Whisper (Offline)</SelectItem>
               </SelectContent>
            </Select>
               ...
               <SettingsSection>
                  ...
                  <p className='text-[11px] text-muted-foreground mt-1'>
                     ...
                     <a
                        ...
                        onClick={(e) => {
                           e.preventDefault();
                           window.__TAURI__.opener.openUrl('https://console.deepgram.com/signup');
                        }}
                     >
            ...
            {form.translation_mode === 'assemblyai' && (
               <SettingsSection>
                  <div className='flex items-center gap-2'>
                     <Label className='text-xs text-muted-foreground'>AssemblyAI API Key</Label>
                     <span className='text-[10px] font-medium text-chart-3 bg-chart-3/10 px-1.5 py-0.5 rounded'>
                        Required
                     </span>
                  </div>
                  <div className='flex gap-1.5'>
                     ...
                     >
                        {showAssemblyAIKey ? <EyeOff className='h-3.5 w-3.5' /> : <Eye className='h-3.5 w-3.5' />}
                     </Button>
                  </div>
                  <p className='text-[11px] text-muted-foreground mt-1'>
                     Get key at{' '}
                     <a
                        ...
                        onClick={(e) => {
                           e.preventDefault();
                           window.__TAURI__.opener.openUrl('https://www.assemblyai.com/dashboard/signup');
                        }}
                     >
                        assemblyai.com
                     </a>{' '}
         ...
         <SettingsSection>
            ...
            {form.translation_engine === 'llm' && (
               <SettingsSection>
                  <div className='flex items-center gap-2'>
                     <Label className='text-xs text-muted-foreground'>LLM API Key</Label>
                     <span className='text-[10px] font-medium text-chart-3 bg-chart-3/10 px-1.5 py-0.5 rounded'>
                        Required
                     </span>
                  </div>
                  ...
                  <Label className='text-xs text-muted-foreground mt-3'>Provider</Label>
                  <LLMProviderSelect
                     form={form}
                     update={update}
                  />
                  ...
                  <p className='text-[11px] text-muted-foreground mt-1'>
                     Select a provider above — API key and model will be pre-filled. Use "Custom" for other
         ...
         <SettingsSection>
            ...
            <div className='flex items-center justify-between mt-3'>
               <span className='text-[11px] text-muted-foreground'>Translation terms</span>
               <Button
                  variant='ghost'
                  size='icon-sm'
...
function LLMProviderSelect({ form, update }: { form: FormData; update: Props['update'] }) {
   return (
      <Select
         value={
            form.llm_base_url === 'https://api.openai.com/v1'
               ...
               : form.llm_base_url === 'https://api.groq.com/openai/v1'
                 ? 'groq'
                 : form.llm_base_url === 'https://generativelanguage.googleapis.com/v1beta/openai'
                   ? 'gemini'
...
function LLMModelSelect({ form, update }: { form: FormData; update: Props['update'] }) {
   const knownUrls = [
   ...
   return (
      <>
         ...
         {models ? (
            <Select
               value={models.some((m) => m.value === form.llm_model) ? form.llm_model : '__custom__'}
               onValueChange={(v) => {
